...
class Input {
    ...
    enum class Key(internal val value: Int) {
        KEY_SPACE (32),
        KEY_APOSTROPHE (39),
        KEY_COMMA (44),
        KEY_MINUS (45),
    ...
    fun mouseState(button: Button): InputState {
        return mouseStates[button.value]
    }

    internal fun putKeyState(key: Int, inputState: InputState) {
        keyboardStates[key] = inputState
        lastFrameKeyState.add(key)
    }
    ...
    internal fun updateKeyState() {
        ...
        for (key in lastFrameKeyState) {
            ...
            else if (keyboardStates[key] == InputState.RELEASED) {
                keyboardStates[key] = InputState.UP
            }
        }

        for (button in lastFrameMouseState) {
            if (mouseStates[button] == InputState.PRESSED) {
                mouseStates[button] = InputState.DOWN
            }
            else if (mouseStates[button] == InputState.RELEASED) {
                mouseStates[button] = InputState.UP
            }
        }
